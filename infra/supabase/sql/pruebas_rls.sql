...
--
-- Correr DESPUÉS de 001_esquema.sql, y otra vez después de cualquier cambio:
--
--   cd /opt/supabase
...
-- intenciones y deshace todo. No queda nada en la base.
--
...
do $$ begin
  perform public.puede_publicar('00000000-0000-4000-8000-00000000000a');
  raise notice 'FALLA  04. Sin sesión se puede consultar si un usuario está suspendido';
exception when insufficient_privilege then
  raise notice 'OK     04. Sin sesión no se consulta el estado de un usuario';
end $$;
...
do $$ declare n int; begin
  ...
  if n = 0 then raise notice 'OK     08. No se ven las notas de otra persona';
  else raise notice 'FALLA  08. Se ven las notas de otra persona'; end if;
exception when others then
  raise notice 'FALLA  08. Error inesperado al leer notas: %', sqlerrm;
end $$;

do $$ begin
  insert into public.notas_academicas (usuario_id, payload_cifrado, sal)
  values ('00000000-0000-4000-8000-00000000000b', 'v1.falso', repeat('s', 24));
  raise notice 'FALLA  09. Se pueden guardar notas a nombre de otra persona';
exception
  when insufficient_privilege then
    raise notice 'OK     09. No se guardan notas a nombre de otra persona';
  when unique_violation then
    raise notice 'FALLA  09. El intento llegó hasta la tabla (lo frenó la clave primaria, no la seguridad)';
end $$;
...
do $$ begin
  ...
  raise notice 'FALLA  10. Una cuenta recién creada puede publicar';
exception when insufficient_privilege then
  raise notice 'OK     10. Una cuenta recién creada no puede publicar en las primeras 24 h';
end $$;
...
do $$ declare n int; begin
  ...
  values ('00000000-0000-4000-8000-00000000000a', 'Turbina de prueba', 'Instrumental', '$10.000', 'Usada', 'Hall');
  get diagnostics n = row_count;
  if n = 1 then raise notice 'OK     11. Pasadas las 24 h se puede publicar';
  else raise notice 'FALLA  11. Pasadas las 24 h no se pudo publicar'; end if;
exception when others then
  raise notice 'FALLA  11. Pasadas las 24 h no se pudo publicar: %', sqlerrm;
end $$;
...
do $$ begin
  ...
  raise notice 'FALLA  12. Se puede publicar con vencimiento a elección';
exception when insufficient_privilege then
  raise notice 'OK     12. No se puede elegir el vencimiento de una publicación';
end $$;

-- Moderación oculta la publicación (lo simula postgres)
reset role;
...
do $$ declare n int; begin
  ...
  where usuario_id = '00000000-0000-4000-8000-00000000000a';
  get diagnostics n = row_count;
  if n = 0 then raise notice 'OK     13. No se reactiva una publicación ocultada por moderación';
  else raise notice 'FALLA  13. Se reactivó una publicación ocultada por moderación'; end if;
exception when others then
  raise notice 'OK     13. No se reactiva una publicación ocultada por moderación (%)', sqlerrm;
end $$;
...
do $$ declare n int; begin
  ...
  where usuario_id = '00000000-0000-4000-8000-00000000000a';
  get diagnostics n = row_count;
  if n = 0 then raise notice 'OK     14. No se borra una publicación ocultada por moderación';
  else raise notice 'FALLA  14. Se borró una publicación ocultada por moderación'; end if;
exception when others then
  raise notice 'OK     14. No se borra una publicación ocultada por moderación (%)', sqlerrm;
end $$;
...
do $$ declare n int; begin
  ...
  get diagnostics n = row_count;
  if n = 1 then raise notice 'OK     15. Se registra el propio consentimiento';
  else raise notice 'FALLA  15. No se pudo registrar el consentimiento'; end if;
exception when others then
  raise notice 'FALLA  15. No se pudo registrar el consentimiento: %', sqlerrm;
end $$;
...
do $$ begin
  ...
  raise notice 'FALLA  16. Se puede antedatar un consentimiento';
exception when insufficient_privilege then
  raise notice 'OK     16. No se puede antedatar un consentimiento';
end $$;

do $$ begin
  perform public.eliminar_mi_cuenta();
  raise notice 'OK     17. Se puede eliminar la propia cuenta';
exception when others then
  raise notice 'FALLA  17. No se pudo eliminar la propia cuenta: %', sqlerrm;
end $$;
...
do $$ begin
  ...
  then
    raise notice 'OK     18. Eliminar la cuenta borra todo lo suyo y nada de nadie más';
  else
    raise notice 'FALLA  18. La eliminación dejó datos propios o tocó datos ajenos';
  end if;
end $$;
...
\echo
\echo 'Pruebas terminadas. Todo se deshizo con ROLLBACK: no quedó nada en la base.'
